...
class DictAggregatorInput(Input):
    """Dictionary Aggregator input model."""

    input_term: str | dict = Field(
        default="$assets",
...
class DictAggregatorExtract(ExtractionMethod):
    ...
    Processor Name: ``dict_aggregator``
        ...
        for value in self.input.input_term.values():
            for list_term in self.input.list_terms:
                if list_term.key in value:
                    body.setdefault(list_term.output_key, []).append(value[list_term.key])

            for sum_term in itertools.chain(self.input.sum_terms, self.input.mean_terms):
